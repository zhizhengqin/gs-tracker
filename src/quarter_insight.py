...
# quarter -> in-flight generation Task (process-wide dedupe)
_insight_tasks: dict = {}
...
async def generate_quarter_insight(
    quarter: str, previous: Optional[str] = None, force: bool = False
) -> dict:
    """Return (or generate) the AI insight report for `quarter` (YYYY-QN)."""
    if not force:
        cached = await asyncio.to_thread(get_quarter_insight, quarter)
        if cached:
    ...
    if previous:
        previous_holdings = await asyncio.to_thread(get_holdings, GOLDMAN_CIK, previous)
    ...
    try:
        ...
        prompt = (
            "你是一位资深的高盛情报分析师。请基于以下高盛 13F 持仓数据，"
        ...
        await asyncio.to_thread(save_quarter_insight, quarter, report)
...
async def ensure_quarter_insight(
    ...
    """
    if await asyncio.to_thread(get_quarter_insight, quarter):
        return None
    task = _insight_tasks.get(quarter)
    if task is None or task.done():
        task = asyncio.create_task(_generate_safe(quarter, previous))
        _insight_tasks[quarter] = task
    return task
